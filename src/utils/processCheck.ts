...
export function isServiceRunning(): boolean {
    ...
    try {
        if (process.platform === 'win32') {
            ...
            // stdio: 'pipe' suppresses command output to prevent it from displaying in console
            const command = `tasklist /FI "PID eq ${pid}"`;
            const output = execSync(command, { stdio: 'pipe' }).toString();

            // If the output contains the PID, the process exists
            // tasklist returns "INFO: No tasks are running..." when no process is found
            // So a simple contains check is sufficient
            if (output.includes(pid.toString())) {
                return true;
            } else {
                // Theoretically, this branch won't be hit if tasklist succeeds but finds nothing
                // But as a safeguard, we still consider the process non-existent
                cleanupPidFile();
                return false;
            }
